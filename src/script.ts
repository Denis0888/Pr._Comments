...
function showComments(): void {
    let resultComment: HTMLElement = document.getElementById('result-comment');
    resultComment.innerHTML = ''
    comments.forEach((item: any, index: number): void => {
        let out: string = `<div class="image-sent" style= "background-image: url('${item.photoSend}');"></div>
...
function submitAnswer(): void {
    let submitAnswer: NodeListOf<HTMLElement> = document.querySelectorAll('.submit-answer');
    submitAnswer.forEach((item: Element) => {
        item.addEventListener('click', (subm: Event): void => {
            subm.preventDefault();
            let answertBody: any = document.getElementById('idAnswer');
            let numAnsw: number = Math.floor(Math.random() * arrUser.length);
            class comAnsw {
                bodyAnswer: string;
                timeAnswer: number;
                userSendAnswer: any;
...
function answerContentDraw(index: number): void {
    ...
    let answerComment: any = comments[index].answer
    answerComment.forEach((item: any, index: number): void => {
        outAnswer +=
...
function toggleHeartAnswer(index: number): void {
    document.querySelectorAll('.inFavoriteAnswer').forEach((item: Element): void => {
        let favoriteBtnAnswer: Element = item;
        item.addEventListener("click", (): void => {
            favoriteBtnAnswer.classList.toggle("toggleHeartAnswer");
            let indexAnswer: string = favoriteBtnAnswer.getAttribute('data-index-answer');
...
function changeRatingAnswer(index: number): void {
    document.querySelectorAll('.rating-answer').forEach((item: Element): void => {
        let btnAns: Element = item;
        item.addEventListener("click", (): void => {
            let indRatAns: string = btnAns.getAttribute('data-index-change-answer');
            ...
            let ratingScore: HTMLElement = document.querySelector(`.rating-text-answer${indRatAns}`);
...
//ОТРИСОВКА "ЛАЙК"
function paintHeart(like: boolean): string {
...
function countComments(): void {
    let text: HTMLElement = document.querySelector(".header-tabs_item-first-num");
...
function showUserCom(num: number): void {
    let img: HTMLElement =  document.querySelector(".user-photo");
    img.style.backgroundImage = `url("${arrUser[num].img}")`;
    let name: HTMLElement = document.querySelector(".position-lable");
